...
//	int32_t   i;
	...
	static char cnt;
	#if LOG_PRINTF_DEF
	LogPrint("rs485DataHandler task start");
	#else
	printf("rs485DataHandler task start\r\n");
	#endif


	tx_buffer[1]=0x03; ///function code
	tx_buffer[2]=0x00; ///register address high byte
	tx_buffer[3]=0x01; ///register address low byte
	tx_buffer[4]=0x00; ///register quantity high byte
	...
	while(1)
	{
		...
		{
			case 0:
				tx_buffer[0]=0x01;//addr = 1
				tx_buffer[5]=0x04;
				status_cnt=1;
				break;
			case 1:
				tx_buffer[0]=0x02;//addr = 2
				tx_buffer[5]=0x06;
				status_cnt=2;
				break;
			case 2:
				tx_buffer[0]=0x03;//addr = 3
				tx_buffer[5]=0x06;
				status_cnt=3;
				break;
			case 3:
				tx_buffer[0]=0x04;//addr = 4
				tx_buffer[5]=0x06;
				status_cnt=4;
				break;
			case 4:
				tx_buffer[0]=0x05;//addr = 5
				tx_buffer[5]=0x06;
				status_cnt=0;
				break;
		}

		crc_value = ModBusCRC (&tx_buffer[0], 6);
		tx_buffer[6] = (unsigned char)(crc_value>>8);
		tx_buffer[7] = (unsigned char)(crc_value&0xff);
#ifdef PRINTF
		printf("\r\ntx buffer hex:");
		for (i=0; i<8; i++)
		{
...
#endif
		HAL_UART_Transmit(&huart2,tx_buffer,8,200);
		...
		{
			cnt = 0;
			LogPrint("----rs485DataHandler task----");
		}

        if(uart2_recv_end_flag ==1)                                           //recv_end_flag 结束标志
        {
//			printf("uart2_rx_len=%d\r\n",uart2_rx_len);                    //rx_len 此次接收到了多少数据
//				for(i=0;i<rx_len;i++)
//				{
//								printf("%x\r\n", uart4_rx_buffer[i]);
//				}
					...
					{
						printf("%02x ", uart2_rx_buffer[i]);
					}
					printf("\r\n");
		#endif

			recvHandler(uart2_rx_buffer);//数据的处理
			uart2_rx_len=0;
...
#endif
	//先发一个数据包激活激光雷达：55 05 00 00 00 00 CC AA
	while(1){
		uint8_t Tx_buf[8];
#if TOF
		while(1){
		...
		Tx_buf[6] = 0xCC;
		//Tx_buf[6] = 0x9f;
		HAL_UART_Transmit(&huart2,Tx_buf,8,200);
		vTaskDelay(1000); /////延迟25ms避免丢失数据
		HAL_UART_Receive_DMA(&huart2, uart2_rx_buffer, UART2_BUF_SIZE);
		...
		while(1){
				HAL_UART_Receive_DMA(&huart2, uart2_rx_buffer, UART2_BUF_SIZE);
				//vTaskDelay(250); /////延迟25ms避免丢失数据
//				if(uart2_rx_buffer[0] != 0x55){
//					break;
//				}
		}
		//接收到的数据包：55 07 00 00 05 AD 9C AA
	}
}
...
//			LP20_Data_Handle();
		...
		sprintf(speed_value,"%c%c%c",uart2_rx_buffer[1],uart2_rx_buffer[2],uart2_rx_buffer[3]);
#endif
	}
}
